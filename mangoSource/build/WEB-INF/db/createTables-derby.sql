...
-- Create a table to store information about report instance points.
create table reportInstancePoints (
  id int not null generated by default as identity (start with 1, increment by 1),  -- Primary key, automatically incremented
  reportInstanceId int not null,                                                  -- Foreign key that links to the main report instance table
  dataSourceName varchar(40) not null,                                            -- Name of the data source
  pointName varchar(100) not null,                                                -- Name of the data point
  dataType int not null,                                                          -- Type of data (numeric code indicating data type)
  startValue varchar(4096),                                                       -- Initial value of the data point
  textRenderer blob,                                                              -- Blob for storing text rendering information
  colour varchar(6),                                                              -- Color associated with the data point, stored as a HEX code
...
create table reportInstanceData (
  ...
  reportInstancePointId int not null,                                             -- Corresponding point ID from reportInstancePoints
  pointValue double,                                                              -- Numeric value of the data point
  ts bigint not null                                                              -- Timestamp of the data point
);

-- Add a composite primary key to reportInstanceData table, initially using pointValueId and reportInstancePointId.
alter table reportInstanceData add constraint reportInstanceDataPk primary key (pointValueId, reportInstancePointId);

-- Add a foreign key constraint to ensure integrity between reportInstanceData and reportInstancePoints.
...
-- Alter the primary key: dropping the existing constraint.
alter table reportInstanceData
drop constraint reportInstanceDataPk;

-- Alter the primary key: adding a new primary key with columns in a different order.
alter table reportInstanceData
